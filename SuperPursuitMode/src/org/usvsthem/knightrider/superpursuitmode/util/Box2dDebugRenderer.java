package org.usvsthem.knightrider.superpursuitmode.util;
...
/*
	...
	*/
	private Shape createPolygonShape(Fixture fixture) {
		...
		float[] bufferData = new float[vSize*3];
		int xIndex;
		int yIndex;
		for(int i = 0; i < fixtureShape.getVertexCount(); i++) {
			...
			bufferData[xIndex] = vertex.x * PhysicsConnector.PIXEL_TO_METER_RATIO_DEFAULT;
			bufferData[yIndex] = vertex.y * PhysicsConnector.PIXEL_TO_METER_RATIO_DEFAULT;
			//bufferData[xIndex] = vertex.x * 1;
			//bufferData[yIndex] = vertex.y * 1;
			
		
		}
		
		return new Path(0,0,bufferData,mVertexBufferObjectManager);
